...
public class ProtocoloAcuerdo1 implements EDProtocol {
   ...
   public Usuario usuario;
	...
	public ProtocoloAcuerdo1(String prefix){
   ...
   //soy el servidor y me llegaron las etiquetas desde un usuario
	 public int procesarEtiquetas(Mensaje msj){
			...
			debug("Me llegaron las respuestas de "+sender.getID());
			for(int i = 0; i < respuestas.tareas.size(); i++){
				debug("("+respuestas.tareas.get(i).id+")"+respuestas.tareas.get(i).respuestasToString());
			}
   ...
   public void recibirImagen(Mensaje msj){
      ...
      if(imagen.getName().equals(usuario.getActual().imagen.getName())){
         ...
         //si no me quedan tareas
            ...
            Mensaje nuevo = new Mensaje();
						int tipo;
						if(usuario.getSiguiente()==Utils.SERVER){
							nuevo.setLabel("ISP_SEND_LABELS");
							tipo = 2;
						}
						else{
							nuevo.setLabel("RESEND_ANSWERS");
							tipo = 6;
						}
            nuevo.setSender(node);
						...
						//ya no uso esto en el servidor por que se pierde en el camino al hacer la redirección
						usuario.lista.pedirMas(CommonState.r.nextFloat()<=Constantes.P_CONTINUAR);
            nuevo.setContenido(usuario.getTareas(), usuario.getTareas().getSizeRespondida());
						int localComDelay = Utils.localDelay();
						Utils.addLatenciaRed(localComDelay, this.node.getID(),tipo, nuevo.getSize());

						//TODO: hasta ahora estaba enviando las respuestas al servidor???????????
						//debo enviarla a otro nodo, segun lo que indica "msj.siguiente"
						//EDSimulator.add(delay, nuevo, Utils.SERVER, Utils.MID);
   ...
   private void sendUnknownPeer(int delay, Mensaje msj, BigInteger  pointOnRing){
	...
	public Object clone(){
		ProtocoloAcuerdo1 pa = null;
		try {
            pa = (ProtocoloAcuerdo1) super.clone();
